...
// v6.4 – Nebenkosten
const SURCHARGE_KEY="gpk_demo_surcharges_v1";
const defaultSurcharges=[
 {id:201,provider:"LIT",type:"Maut",value:48,unit:"EUR",country:"DE",zone:"*",transport:"FTL",from:"2026-01-01",to:"2026-12-31",status:"active",notes:"pauschal"},
 {id:202,provider:"LIT",type:"Wartezeit",value:65,unit:"EUR",country:"*",zone:"*",transport:"*",from:"2026-01-01",to:"2026-12-31",status:"active",notes:"je angefangene Stunde"},
 {id:203,provider:"Transimeksa",type:"ADR",value:7.5,unit:"percent",country:"BE",zone:"*",transport:"*",from:"2026-01-01",to:"2026-12-31",status:"active",notes:"Gefahrgut"},
 {id:204,provider:"Dachser",type:"Avis",value:25,unit:"EUR",country:"FR",zone:"*",transport:"*",from:"2026-01-01",to:"2026-12-31",status:"active",notes:"Terminavis"}
];
...
function renderSurcharges(){const q=surchargeSearch.value.trim().toLowerCase(),pf=surchargeProviderFilter.value,sf=surchargeStatusFilter.value;const f=surcharges.filter(x=>{const h=`${x.provider} ${x.type} ${x.country} ${x.zone} ${x.transport} ${x.notes}`.toLowerCase();return(!q||h.includes(q))&&(!pf||x.provider===pf)&&(!sf||x.status===sf)});activeSurchargeCount.textContent=surcharges.filter(x=>x.status==="active").length;surchargeProviderCount.textContent=new Set(surcharges.map(x=>x.provider)).size;surchargeTypeCount.textContent=new Set(surcharges.map(x=>x.type)).size;surchargeHistoryCount.textContent=surcharges.length;visibleSurchargeCount.textContent=f.length;surchargeRows.innerHTML=f.map(x=>`<tr><td><strong class="table-main">${esc(x.provider)}</strong></td><td><strong class="table-main">${esc(x.type)}</strong><small>${esc(x.notes||"—")}</small></td><td><strong>${esc(x.country||"*")}</strong><small>${esc(x.zone||"*")} · ${esc(x.transport||"*")}</small></td><td><strong class="price-cell">${x.unit==="percent"?Number(x.value).toLocaleString("de-DE")+" %":euro(x.value)}</strong></td><td><strong class="table-main">${isoToDE(x.from)} – ${isoToDE(x.to)}</strong></td><td><span class="status-pill ${x.status}">${x.status==="active"?"Aktiv":"Inaktiv"}</span></td><td class="row-actions"><button class="icon-button" data-edit-surcharge="${x.id}">✎</button></td></tr>`).join("")||`<tr><td colspan="7" class="empty-state">Keine Nebenkosten für diesen Filter gefunden.</td></tr>`;}
function openSurchargeModal(x=null){editingSurchargeId=x?.id??null;surchargeModalTitle.textContent=x?"Nebenkosten bearbeiten":"Neue Nebenkosten";surchargeProvider.value=x?.provider??providerNames[0];surchargeType.value=x?.type??"Maut";surchargeValue.value=x?.value??"";surchargeUnit.value=x?.unit??"EUR";surchargeCountry.value=x?.country??"*";surchargeZone.value=x?.zone??"*";surchargeTransport.value=x?.transport??"*";surchargeFrom.value=x?.from??"";surchargeTo.value=x?.to??"";surchargeStatus.value=x?.status??"active";surchargeNotes.value=x?.notes??"";surchargeModal.hidden=false;document.body.classList.add("modal-open");}
function closeSurcharge(){surchargeModal.hidden=true;document.body.classList.remove("modal-open")}
newSurchargeBtn.addEventListener("click",()=>openSurchargeModal());closeSurchargeModalBtn.addEventListener("click",closeSurcharge);cancelSurchargeModalBtn.addEventListener("click",closeSurcharge);surchargeModal.addEventListener("click",e=>{if(e.target===surchargeModal)closeSurcharge()});surchargeRows.addEventListener("click",e=>{const b=e.target.closest("[data-edit-surcharge]");if(!b)return;const x=surcharges.find(v=>v.id===Number(b.dataset.editSurcharge));if(x)openSurchargeModal(x)});surchargeForm.addEventListener("submit",e=>{e.preventDefault();const data={provider:surchargeProvider.value,type:surchargeType.value,value:Number(surchargeValue.value||0),unit:surchargeUnit.value,country:surchargeCountry.value.trim()||"*",zone:surchargeZone.value.trim()||"*",transport:surchargeTransport.value,from:surchargeFrom.value,to:surchargeTo.value,status:surchargeStatus.value,notes:surchargeNotes.value.trim()};if(editingSurchargeId)Object.assign(surcharges.find(x=>x.id===editingSurchargeId),data);else surcharges.unshift({id:Date.now(),...data});GPK.write(SURCHARGE_KEY,surcharges);closeSurcharge();renderSurcharges();toast("Nebenkosten gespeichert.");});[surchargeSearch,surchargeProviderFilter,surchargeStatusFilter].forEach(x=>x.addEventListener("input",renderSurcharges));initSurchargeProviders();renderSurcharges();
