const masterProviderRecords=(()=>{const stored=GPK.read(GPK.KEYS.providers,[])||[];if(Array.isArray(stored)&&stored.length)return stored;return [
{id:1,name:"LIT",status:"active"},{id:2,name:"Transimeksa",status:"active"},{id:3,name:"Bertschi",status:"active"},{id:4,name:"Duvenbeck",status:"active"},{id:5,name:"Dachser",status:"active"},{id:6,name:"Kuehne + Nagel",status:"inactive"},{id:7,name:"Raben",status:"active"},{id:8,name:"DSV",status:"active"}
];})();const providerNames=[...new Set(masterProviderRecords.map(p=>p.name).filter(Boolean))];const defaultRates=[{id:1,provider:"LIT",country:"DE",zone:"24",zipFrom:"24000",zipTo:"24999",transport:"FTL",ldm:"",base:920,floater:8.5,status:"active"},{id:2,provider:"LIT",country:"DE",zone:"22",zipFrom:"22000",zipTo:"22999",transport:"Mega",ldm:"",base:995,floater:8.5,status:"active"},{id:3,provider:"Transimeksa",country:"BE",zone:"24",zipFrom:"2400",zipTo:"2499",transport:"FTL",ldm:"",base:1180,floater:7,status:"active"},{id:4,provider:"Bertschi",country:"DE",zone:"48",zipFrom:"48000",zipTo:"48999",transport:"Teilladung",ldm:"5,0",base:680,floater:6.5,status:"active"},{id:5,provider:"Duvenbeck",country:"NL",zone:"50",zipFrom:"5000",zipTo:"5099",transport:"FTL",ldm:"",base:1240,floater:9,status:"active"},{id:6,provider:"Dachser",country:"FR",zone:"67",zipFrom:"67000",zipTo:"67999",transport:"Jumbo",ldm:"",base:1490,floater:8,status:"active"},{id:7,provider:"Raben",country:"DE",zone:"47",zipFrom:"47000",zipTo:"47999",transport:"Teilladung",ldm:"7,5",base:755,floater:7.5,status:"active"},{id:8,provider:"DSV",country:"DE",zone:"22",zipFrom:"22000",zipTo:"22999",transport:"FTL",ldm:"",base:1015,floater:8.2,status:"inactive"},{id:9,provider:"LIT",country:"DE",zone:"24",zipFrom:"24000",zipTo:"24999",transport:"Teilladung",ldm:"3,0",base:545,floater:8.5,status:"active"},{id:10,provider:"Transimeksa",country:"DE",zone:"24",zipFrom:"24000",zipTo:"24999",transport:"Mega",ldm:"",base:980,floater:7,status:"active"}];let rates=GPK.read(GPK.KEYS.rates,null)||defaultRates;let editingId=null;const rows=document.getElementById("rateRows"),search=document.getElementById("rateSearch"),providerFilter=document.getElementById("rateProviderFilter"),transportFilter=document.getElementById("rateTransportFilter"),statusFilter=document.getElementById("rateStatusFilter"),sortFilter=document.getElementById("rateSort"),modal=document.getElementById("rateModal");function esc(v=""){return String(v).replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]))}function providerRecord(name){return masterProviderRecords.find(p=>String(p.name||"").trim()===String(name||"").trim())||null}function providerLogoSrc(p){const logo=String(p?.logo||"").trim();if(!logo)return "";if(logo.startsWith("data:")||logo.startsWith("blob:"))return logo;return logo.includes("/")?logo:`logos/${logo}`}function providerAvatarHtml(name){const p=providerRecord(name),src=providerLogoSrc(p),initials=String(p?.alias||name||"?").replace(/[^A-Za-z0-9]/g,"").slice(0,2).toUpperCase()||"?";return src?`<div class="provider-avatar provider-avatar-logo"><img src="${esc(src)}" alt="${esc(name)} Logo" onerror="this.parentElement.classList.remove('provider-avatar-logo');this.remove();this.parentElement.textContent='${esc(initials)}'"></div>`:`<div class="provider-avatar">${esc(initials)}</div>`}function euro(n){return new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR",maximumFractionDigits:0}).format(n)}function total(r){return r.base*(1+(Number(r.floater)||0)/100)}function initProviders(){providerNames.forEach(p=>{let a=document.createElement("option");a.value=p;a.textContent=p;providerFilter.appendChild(a);let b=document.createElement("option");b.value=p;b.textContent=p;rateProvider.appendChild(b)})}function render(){let q=search.value.trim().toLowerCase(),pf=providerFilter.value,tf=transportFilter.value,sf=statusFilter.value;let filtered=rates.filter(r=>{let hay=`${r.provider} ${r.country} ${r.zone} ${r.zipFrom} ${r.zipTo} ${r.transport}`.toLowerCase();return(!q||hay.includes(q))&&(!pf||r.provider===pf)&&(!tf||r.transport===tf)&&(!sf||r.status===sf)}).sort((a,b)=>{const mode=sortFilter?.value||"newest";if(mode==="provider")return String(a.provider||"").localeCompare(String(b.provider||""),"de",{sensitivity:"base"});if(mode==="provider_desc")return String(b.provider||"").localeCompare(String(a.provider||""),"de",{sensitivity:"base"});const ad=new Date(a.importedAt||a.createdAt||0),bd=new Date(b.importedAt||b.createdAt||0);return mode==="oldest"?ad-bd:bd-ad;});activeRateCount.textContent=rates.filter(r=>r.status==="active").length;rateProviderCount.textContent=new Set(rates.map(r=>r.provider)).size;rateCountryCount.textContent=new Set(rates.map(r=>r.country)).size;visibleRateCount.textContent=filtered.length;rows.innerHTML=filtered.map(r=>`<tr data-sort-provider="${esc(r.provider)}" data-sort-date="${esc(r.importedAt||r.createdAt||"")}"><td><div class="provider-name-cell">${providerAvatarHtml(r.provider)}<div><strong>${esc(r.provider)}</strong><small>Tarif-ID ${r.id}</small></div></div></td><td><strong class="table-main">${esc(r.country)}</strong><small>${esc(r.zipFrom)}–${esc(r.zipTo)}</small></td><td><strong class="table-main">Zone ${esc(r.zone)}</strong><small>${esc(r.zipFrom)}–${esc(r.zipTo)}</small></td><td><span class="transport-pill">${esc(r.transport)}</span>${r.ldm?`<small>${esc(r.ldm)} Ldm</small>`:""}</td><td><span class="floater-pill">${Number(r.floater).toLocaleString("de-DE")} %</span></td><td><span class="table-muted">${r.importedAt?new Date(r.importedAt).toLocaleString("de-DE"):"—"}</span></td><td><strong class="table-main">${esc(r.version||"MAN")}</strong></td><td><span class="status-pill ${r.status}">${r.status==="active"?"Aktiv":"Inaktiv"}</span></td><td class="row-actions"><button class="icon-button" data-edit="${r.id}" title="Tarif bearbeiten">✎</button><button class="icon-button more-button" data-rate-menu="${r.id}" title="Weitere Aktionen">•••</button><button class="icon-button danger-icon" data-delete-rate="${r.id}" title="Tarif löschen">🗑</button></td></tr>`).join("")||`<tr><td colspan="9" class="empty-state">Keine Tarife für diesen Filter gefunden.</td></tr>`}function toast(text){rateToast.textContent=text;rateToast.hidden=false;clearTimeout(toast.timer);toast.timer=setTimeout(()=>rateToast.hidden=true,2400)}function openModal(r=null){editingId=r?.id??null;rateModalTitle.textContent=r?"Tarif bearbeiten":"Neuer Tarif";rateProvider.value=r?.provider??providerNames[0];rateStatus.value=r?.status??"active";rateCountry.value=r?.country??"DE";rateZone.value=r?.zone??"";rateZipFrom.value=r?.zipFrom??"";rateZipTo.value=r?.zipTo??"";rateTransport.value=r?.transport??"FTL";rateLdm.value=r?.ldm??"";rateBase.value=r?.base??"";rateFloater.value=r?.floater??"";rateNotes.value=r?.notes??"";modal.hidden=false;document.body.classList.add("modal-open")}function closeModal(){modal.hidden=true;document.body.classList.remove("modal-open")}newRateBtn.addEventListener("click",()=>openModal());closeRateModalBtn.addEventListener("click",closeModal);cancelRateModalBtn.addEventListener("click",closeModal);rows.addEventListener("click",e=>{
  const edit=e.target.closest("[data-edit]");
  if(edit){const r=rates.find(x=>x.id===Number(edit.dataset.edit));if(r)openModal(r);return;}
  const del=e.target.closest("[data-delete-rate]");
  if(del){const id=Number(del.dataset.deleteRate);const r=rates.find(x=>x.id===id);if(r&&window.confirm(`Tarif von ${r.provider} wirklich löschen?`)){rates=rates.filter(x=>x.id!==id);GPK.write(GPK.KEYS.rates,rates);render();toast("Tarif wurde gelöscht.");}return;}
  const more=e.target.closest("[data-rate-menu]");
  if(more)openRateMenu(more,Number(more.dataset.rateMenu));
});rateForm.addEventListener("submit",e=>{e.preventDefault();let data={provider:rateProvider.value,status:rateStatus.value,country:rateCountry.value,zone:rateZone.value.trim(),zipFrom:rateZipFrom.value.trim(),zipTo:rateZipTo.value.trim(),transport:rateTransport.value,ldm:rateLdm.value.trim(),base:Number(rateBase.value||0),floater:Number(rateFloater.value||0),notes:rateNotes.value.trim(),version:editingId?(rates.find(x=>x.id===editingId)?.version||"MAN"):"MAN"};if(editingId){Object.assign(rates.find(x=>x.id===editingId),data);toast("Tarif wurde in der Layout-Demo aktualisiert.")}else{rates.unshift({id:Date.now(),...data});toast("Tarif wurde in der Layout-Demo angelegt.")}GPK.write(GPK.KEYS.rates,rates);closeModal();render()});[search,providerFilter,transportFilter,statusFilter,sortFilter].filter(Boolean).forEach(x=>{x.addEventListener("input",render);x.addEventListener("change",render);});importRatesBtn.addEventListener("click",()=>GPKTariffImport.chooseAndOpen());exportRatesBtn.addEventListener("click",async ()=>{
  try{
...
function ensureProviderOption(select,name){if(!select||!name)return;if(![...select.options].some(o=>o.value===name)){const o=document.createElement("option");o.value=name;o.textContent=name;select.appendChild(o);}}
function openFloaterForProvider(provider){
  const name=String(provider||"").trim();if(!name)return;
  [floaterProviderFilter,floaterProvider,providerFilter,rateProvider,surchargeProviderFilter,surchargeProvider].forEach(sel=>ensureProviderOption(sel,name));
  tariffSubtabs.forEach(x=>x.classList.toggle("active",x.dataset.tariffTab==="floater"));
  tariffRatesPanel.hidden=true;tariffFloaterPanel.hidden=false;document.getElementById("tariffSurchargePanel").hidden=true;
  floaterProviderFilter.value=name;renderFloaters();
  const today=new Date().toISOString().slice(0,10);const current=floaterPeriods.filter(x=>x.provider===name&&(!x.from||x.from<=today)&&(!x.to||x.to>=today)).sort((a,b)=>String(b.from).localeCompare(String(a.from)))[0];
  openFloaterModal(current||null);floaterProvider.value=name;
}
window.GPKTariffUI={openFloaterForProvider};
(function handleTariffPageQuery(){const q=new URLSearchParams(location.search);const provider=q.get("provider"),tab=q.get("tab");if(tab==="floater"&&provider){setTimeout(()=>openFloaterForProvider(provider),0);}})();
