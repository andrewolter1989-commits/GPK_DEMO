...
function providerTariffCountries(providerName){
  ...
  return [...new Set(allStoredRates()
    .filter(r=>normalizedProviderName(rateProviderName(r))===needle)
    .map(r=>String(rateCountry(r)||"").trim().toUpperCase())
    .filter(Boolean))].sort();
}
...
function currentProviderFloater(provider){
  ...
  const periods=(GPK.read(GPK.KEYS.floaters,[])||[]).filter(x=>normalizedProviderName(x?.provider ?? x?.provider_name)===needle);
  if(!periods.length)return provider?.floater||"";
  const today=new Date().toISOString().slice(0,10);
  const norm=x=>({
    from:String(x?.from ?? x?.valid_from ?? "").slice(0,10),
    to:String(x?.to ?? x?.valid_to ?? "").slice(0,10),
    value:Number(x?.value ?? x?.floater ?? x?.floater_percent ?? 0)
  });
  const normalized=periods.map(norm).filter(x=>Number.isFinite(x.value));
  const current=normalized
    .filter(x=>(!x.from||x.from<=today)&&(!x.to||x.to>=today))
    .sort((a,b)=>String(b.from).localeCompare(String(a.from)))[0];
  const latest=current || normalized.sort((a,b)=>String(b.from).localeCompare(String(a.from)))[0];
  return latest ? `${latest.value.toLocaleString("de-DE")} %` : (provider?.floater||"");
}
...
function openProviderContextMenu(button,id){
  ...
  menu.innerHTML=`<button type="button" data-action="stats">Statistiken anzeigen</button><button type="button" data-action="tariffs">Tarife anzeigen</button><button type="button" data-action="edit">Bearbeiten</button><button type="button" data-action="toggle">${p.status==="active"?"Deaktivieren":"Aktivieren"}</button><button type="button" class="danger-menu-action" data-action="delete">Löschen</button>`;
  ...
  menu.addEventListener("click",ev=>{
    ...
    if(action==="stats")location.href=`statistik.html?carrier=${encodeURIComponent(p.name)}&tab=carriers&period=all`;
