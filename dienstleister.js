...
function render(){const q=search.value.trim().toLowerCase(),status=statusFilter.value,rf=rateFilter.value;const filtered=providers.filter(p=>{const contacts=(p.contacts||[]).map(c=>`${c.name} ${c.emails} ${c.phone} ${c.countries}`).join(" ");const hay=`${p.name} ${p.alias} ${p.city||""} ${p.country||""} ${contacts}`.toLowerCase();const rateOK=!rf||(rf==="with"?p.rates>0:p.rates===0);return(!q||hay.includes(q))&&(!status||p.status===status)&&rateOK});providerCount.textContent=providers.length;activeProviderCount.textContent=providers.filter(p=>p.status==="active").length;ratedProviderCount.textContent=providers.filter(p=>p.rates>0).length;missingMailCount.textContent=providers.filter(p=>!(p.contacts||[]).some(c=>String(c.emails||"").trim())).length;visibleProviderCount.textContent=filtered.length;rows.innerHTML=filtered.map(p=>{const c=primaryContact(p);return `<tr><td><div class="provider-name-cell"><div class="provider-avatar">${esc(initials(p))}</div><div><strong>${esc(p.name)}</strong><small>${esc(p.alias||"Kein Alias")} · ${esc(p.country||"—")} ${esc(p.city||"")}</small></div></div></td><td><strong class="table-main">${esc(c.name||"—")}</strong><small>${esc(c.emails||c.phone||"Keine Kontaktdaten")}${(p.contacts||[]).length>1?` · +${p.contacts.length-1} weitere`:""}</small></td><td><strong class="provider-number">${p.rates}</strong><small>${p.rates===1?"Tarif":"Tarife"} hinterlegt</small></td><td><span class="floater-pill">${esc(p.floater||"—")}</span></td><td><span class="status-pill ${p.status}">${p.status==="active"?"Aktiv":"Inaktiv"}</span></td><td class="row-actions"><button class="icon-button" data-edit="${p.id}" title="Bearbeiten">✎</button><button class="icon-button more-button" data-provider-more="${p.id}" title="Weitere Aktionen">•••</button></td></tr>`}).join("")||`<tr><td colspan="6" class="empty-state">Keine Dienstleister für diesen Filter gefunden.</td></tr>`;}
...
function contactRow(c={}){const id=c.id||Date.now()+Math.floor(Math.random()*9999),ps=c.purposes||[];return `<div class="provider-contact-card" data-contact-id="${id}"><div class="provider-contact-top"><strong>Ansprechpartner</strong><button class="icon-button remove-provider-contact" type="button" title="Entfernen">×</button></div><div class="location-form-grid"><div class="field"><label>Name / Funktion</label><input data-contact-field="name" value="${esc(c.name||"")}" placeholder="z. B. Disposition"></div><div class="field"><label>Telefon</label><input data-contact-field="phone" value="${esc(c.phone||"")}" placeholder="+49 ..."></div><div class="field span-2"><label>E-Mail-Adresse(n)</label><input data-contact-field="emails" value="${esc(c.emails||"")}" placeholder="booking@..., dispo@..."><span class="field-hint">Mehrere Adressen durch Komma trennen.</span></div><div class="field span-2"><label>Für welche Länder?</label><input data-contact-field="countries" value="${esc(c.countries||"*")}" placeholder="DE, BE, NL oder * für alle"></div><div class="field span-2"><label>Verwendung</label><div class="contact-purpose-list"><label><input type="checkbox" data-purpose="booking" ${ps.includes("booking")?"checked":""}> Buchungen</label><label><input type="checkbox" data-purpose="availability" ${ps.includes("availability")?"checked":""}> Verfügbarkeitsanfragen</label><label><input type="checkbox" data-purpose="price" ${ps.includes("price")?"checked":""}> Preisanfragen</label></div></div></div></div>`;}
function renderContacts(list=[]){providerContacts.innerHTML=(list.length?list:[{}]).map(contactRow).join("");}
function collectContacts(){return [...providerContacts.querySelectorAll(".provider-contact-card")].map(card=>({id:Number(card.dataset.contactId)||Date.now(),name:card.querySelector('[data-contact-field="name"]').value.trim(),phone:card.querySelector('[data-contact-field="phone"]').value.trim(),emails:card.querySelector('[data-contact-field="emails"]').value.trim(),countries:card.querySelector('[data-contact-field="countries"]').value.trim()||"*",purposes:[...card.querySelectorAll('[data-purpose]:checked')].map(x=>x.dataset.purpose)})).filter(c=>c.name||c.emails||c.phone);}
function openModal(p=null){editingId=p?.id??null;providerModalTitle.textContent=p?"Dienstleister bearbeiten":"Neuer Dienstleister";providerName.value=p?.name??"";providerAlias.value=p?.alias??"";providerStatus.value=p?.status??"active";providerStreet.value=p?.street??"";providerZip.value=p?.zip??"";providerCity.value=p?.city??"";providerCountry.value=p?.country??"DE";providerLogo.value=p?.logo??"";providerNotes.value=p?.notes??"";renderContacts(p?.contacts||[]);deleteProviderBtn.hidden=!p;modal.hidden=false;document.body.classList.add("modal-open");}
