(function(){
  ...
  const PROVIDER_PATTERNS=[
    ...
    ["Emons",/\bemons\b/i],["Oetjen",/\boetjen\b/i],["PML",/\bPML\b/i]
  ];
  ...
  function parseSpDimensionLabel(v){
    ...
    if(!t)return null;
    if(/^\s*ftl\s*$/i.test(t))return {model:"FULL_LOAD",product:"FTL",from:"",to:"",label:raw};
    let m=t.match(/^(\d+(?:[.,]\d+)?)\s*(?:-|–)\s*(\d+(?:[.,]\d+)?)\s*sp$/i);
    if(m)return {model:"PALLET_STEP",product:"Stellplatz",from:num(m[1]),to:num(m[2]),label:raw};
    m=t.match(/^(\d+(?:[.,]\d+)?)\s*sp$/i);
    if(m){const n=num(m[1]);return {model:"PALLET_STEP",product:"Stellplatz",from:n,to:n,label:raw};}
    return null;
  }

  function parseWeightDimensionLabel(v,previousTo=null){
    const raw=text(v),t=lower(raw);
    if(!t)return null;
    if(/min(?:imum|um)|mind(?:est)?/.test(t))return {model:"WEIGHT_STEP",product:"Stückgut",from:"",to:"",label:raw||"Minimum",isMinimum:true};
    let n=null;
    const m=t.match(/(?:bis\s*)?(\d+(?:[.,]\d+)?)/);
    if(m)n=num(m[1]);
    if(n===null)return null;
    const from=previousTo===null||previousTo===""?0:round(Number(previousTo)+0.01,2);
    return {model:"WEIGHT_STEP",product:"Stückgut",from,to:n,label:raw};
  }


  function tariffZoneIdentity(z){
    const manual=Number(z?.manualZoneNumber);
    if(Number.isInteger(manual)&&manual>=1)return `${text(z?.blockId||z?.zoneSet)}|N${manual}`;
    return `${text(z?.blockId||z?.zoneSet)}|K${text(z?.zone)||text(z?.destinationName)||text(z?.sourceRef)}`;
  }
  function countTariffZones(zones){
    return new Set((zones||[]).map(tariffZoneIdentity).filter(Boolean)).size;
  }
  function prefixFromPostcodeRule(z,country){
    const c=text(z?.destCountry||country).toUpperCase(),from=text(z?.destPostcodeFrom),to=text(z?.destPostcodeTo);
    if(!from)return "";
    /* Evers-&-Frank DE-Zonen sind zweistellige PLZ-Gebiete. */
    if(c==="DE"&&from.length>=2){
      const p=from.slice(0,2);
      if(to&&to.slice(0,2)===p)return p;
    }
    const common=to?commonPrefix(from,to):from;
    return (common||from).replace(/0+$/,"")||from;
  }
  function compressIntegerPrefixes(values){
    const nums=[...new Set(values.map(v=>Number(v)).filter(Number.isInteger))].sort((a,b)=>a-b);
    const out=[];
    for(let i=0;i<nums.length;){
      let j=i;while(j+1<nums.length&&nums[j+1]===nums[j]+1)j++;
      if(j-i>=2)out.push(`${String(nums[i]).padStart(2,"0")} - ${String(nums[j]).padStart(2,"0")}`);
      else for(let k=i;k<=j;k++)out.push(String(nums[k]).padStart(2,"0"));
      i=j+1;
    }
    return out.join(", ");
  }
  function deriveZoneAssignment(col,country){
    const prefixes=(col?.rules||[]).map(z=>prefixFromPostcodeRule(z,country)).filter(v=>/^\d{1,2}$/.test(v));
    return prefixes.length?compressIntegerPrefixes(prefixes):"";
  }
  function zoneAssignmentText(block,col,country){
    const n=Number(col?.zoneNumber);
    const stored=text(block?.zoneAssignments?.[n]);
    return stored||deriveZoneAssignment(col,country);
  }
  function rebuildZoneAssignment(block,col,country,expr){
    const zoneNumber=Number(col?.zoneNumber)||1;
    const zoneKey=text(col?.key)||`${country}-Z${zoneNumber}`;
    const c=text(country||col?.rules?.[0]?.destCountry||"DE").toUpperCase();
    const oldRules=(block.zones||[]).filter(z=>text(z.zone)===zoneKey||Number(z.manualZoneNumber)===zoneNumber);
    const oldIds=new Set(oldRules.map(z=>z.id));
    block.zones=(block.zones||[]).filter(z=>!oldIds.has(z.id));
    const expanded=expandPrefixExpression(c,expr);
    expanded.forEach((rg,i)=>block.zones.push({
      id:makeId("zone"),blockId:block.id,zoneSet:block.sheet,ruleType:"POSTCODE",
      originCountry:"",originPostcodeFrom:"",originPostcodeTo:"",
      destCountry:c,destPostcodeFrom:rg.from,destPostcodeTo:rg.to,
      distanceFromKm:"",distanceToKm:"",originName:"",destinationName:"",
      zone:zoneKey,manualZoneNumber:zoneNumber,manualOrder:zoneNumber,
      priority:100,sourceRef:`MANUAL-ZONE-${zoneNumber}-${i+1}`
    }));
    block.zoneAssignments=block.zoneAssignments||{};
    block.zoneAssignments[zoneNumber]=text(expr);
  }

  function makePostcodeZone(blockId,sheetName,country,code,rowNo,manualZoneNumber=null){
    const rg=postcodeRange(country,code);
    return {
      ...
      distanceFromKm:"",distanceToKm:"",originName:"",destinationName:"",
      zone:(country?country+"-":"")+code,manualZoneNumber,
      priority:100,sourceRef:sheetName+"!R"+rowNo
    };
  }

  function parseEversFrankSpMatrix(sheetName,rows){
    /* Evers & Frank / ähnliche Ausschreibungsvorlagen:
       PLZ / SP | 1 Sp | 2 Sp | ... | 16-20 SP | ... | FTL
       Einzelne Preisfelder dürfen leer sein. */
    let headerIdx=-1,zoneCol=-1,dims=[];
    for(let i=0;i<Math.min(rows.length,60);i++){
      const row=rows[i]||[];
      const z=row.findIndex(v=>/^\s*plz\s*\/\s*sp\s*$/i.test(text(v)));
      if(z<0)continue;
      const found=[];
      for(let c=z+1;c<row.length;c++){
        const d=parseSpDimensionLabel(row[c]);
        if(d)found.push({col:c,...d,order:c});
      }
      if(found.length>=2){headerIdx=i;zoneCol=z;dims=found;break;}
    }
  ...
  function parseEversFrankWeightMatrix(sheetName,rows){
    /* Sonderfall derselben Vorlage:
       PLZ / SP | minimum | bis 100 | bis 150 | 200 | ...
       Trotz 'SP' ist die Preisachse hier eine Gewichtsstaffel. */
    let headerIdx=-1,zoneCol=-1,dims=[];
    for(let i=0;i<Math.min(rows.length,60);i++){
      const row=rows[i]||[];
      const z=row.findIndex(v=>/^\s*plz\s*\/\s*sp\s*$/i.test(text(v)));
      if(z<0)continue;
      const after=row.slice(z+1).map(text);
      const spCount=after.filter(v=>parseSpDimensionLabel(v)).length;
      const weightSignals=after.filter(v=>/min(?:imum|um)|^\s*bis\s*\d+|^\s*\d+(?:[.,]\d+)?\s*$/i.test(v)).length;
      if(spCount>=2||weightSignals<3)continue;
      let previousTo=null,found=[];
      for(let c=z+1;c<row.length;c++){
        const d=parseWeightDimensionLabel(row[c],previousTo);
        if(!d)continue;
        found.push({col:c,...d,order:c});
        if(!d.isMinimum&&d.to!=="")previousTo=d.to;
      }
      if(found.length>=3){headerIdx=i;zoneCol=z;dims=found;break;}
    }
    if(headerIdx<0)return null;

    const blockId=makeId("block"),rates=[],zones=[];
    const fallbackCountry=inferCountryFromSheet(sheetName)||"DE";
    let dataRows=0,empty=0;
    for(let r=headerIdx+1;r<rows.length;r++){
      const row=rows[r]||[],label=text(row[zoneCol]);
      const cp=parseCountryPostcode([label],fallbackCountry);
      ...
      empty=0;dataRows++;
      const country=cp.country||fallbackCountry,zone=(country?country+"-":"")+cp.code;
      zones.push(makePostcodeZone(blockId,sheetName,country,cp.code,r+1));
      for(const d of populated){
        const price=num(row[d.col]);if(price===null)continue;
        rates.push({
          id:makeId("rate"),blockId,sheet:sheetName,sourceRow:r+1,manualOrder:d.order,
          rateModel:"WEIGHT_STEP",product:"Stückgut",subservice:sheetName,
          originCountry:"",destCountry:country,zone,relationName:"",
          chargeFrom:d.from,chargeTo:d.to,chargeLabel:d.label,
          unit:"EUR/SHIPMENT",price:round(price),currency:"EUR",priority:100,
          sourceRef:sheetName+"!R"+(r+1)+"C"+(d.col+1)
        });
      }
    }
    ...
    return {
      id:blockId,sheet:sheetName,headerRow:headerIdx+1,model:"WEIGHT_STEP",
      modelLabel:"Gewichtsstaffel",confidence:0.99,status:"AUTO_CANDIDATE",
      rates,zones:dz,summary:`${rates.length} Preise · ${dz.length} Zonen`,
      context:"Evers-&-Frank-Stückgutmatrix: PLZ / SP × Gewicht"
    };
  }

  function parseEversFrankCustomZoneMatrix(sheetName,rows){
    /* Eigene Zoneneinteilung:
       PLZ | Zone 1 | Zone 2 | ...
       darunter PLZ-Präfixe je Zone,
       anschließend vertikale Staffelzeilen (z. B. 6 Sp ... 28-33 SP oder Gewicht). */
    let headerIdx=-1,zoneCols=[];
    for(let i=0;i<Math.min(rows.length,80);i++){
      const row=rows[i]||[],found=[];
      for(let c=0;c<row.length;c++){
        const m=text(row[c]).match(/^\s*zone\s*(\d{1,3})\s*$/i);
        if(m)found.push({col:c,num:Number(m[1]),label:text(row[c])});
      }
      const left=lower(rowText(row.slice(0,Math.max(1,found[0]?.col||1))));
      if(found.length>=2&&/plz/.test(left)){headerIdx=i;zoneCols=found;break;}
    }
    if(headerIdx<0)return null;

    const firstZone=Math.min(...zoneCols.map(z=>z.col));
    let rateStart=-1,model=null;
    for(let r=headerIdx+1;r<rows.length;r++){
      const leftCells=(rows[r]||[]).slice(0,firstZone).filter(v=>text(v)!=="");
      const dimCell=leftCells.length?leftCells[leftCells.length-1]:"";
      if(parseSpDimensionLabel(dimCell)){rateStart=r;model="PALLET_STEP";break;}
      const t=lower(dimCell);
      const n=num(dimCell);
      if(/min(?:imum|um)|mind(?:est)?/.test(t)||(n!==null&&n>=50)){rateStart=r;model="WEIGHT_STEP";break;}
    }
    ...
    for(const zc of zoneCols){
      const zoneName=`${country}-Z${zc.num}`;
      let anyRule=false;const assignmentParts=[];
      for(let r=headerIdx+1;r<rateStart;r++){
        const expr=text((rows[r]||[])[zc.col]);if(!expr)continue;
        assignmentParts.push(expr);
        const ranges=expandPrefixExpression(country,expr);
        for(const rg of ranges){
          ...
          zones.push({
            ...
            destCountry:country,destPostcodeFrom:rg.from,destPostcodeTo:rg.to,
            distanceFromKm:"",distanceToKm:"",originName:"",destinationName:"",
            zone:zoneName,manualZoneNumber:zc.num,priority:100,
            sourceRef:sheetName+"!R"+(r+1)+"C"+(zc.col+1)
          });
        }
      }
      zoneAssignments[zc.num]=assignmentParts.join(", ");
      if(!anyRule){
        zones.push({
          id:makeId("zone"),blockId,zoneSet:sheetName,ruleType:"SERVICE_ZONE",
          originCountry:"",originPostcodeFrom:"",originPostcodeTo:"",
          destCountry:country,destPostcodeFrom:"",destPostcodeTo:"",
          distanceFromKm:"",distanceToKm:"",originName:"",destinationName:"",
          zone:zoneName,manualZoneNumber:zc.num,priority:70,
          sourceRef:sheetName+"!R"+(headerIdx+1)+"C"+(zc.col+1)
        });
      }
    }

    let previousWeightTo=null,dataRows=0,empty=0;
    for(let r=rateStart;r<rows.length;r++){
      const row=rows[r]||[],leftCells=row.slice(0,firstZone).filter(v=>text(v)!=="");
      const dimCell=leftCells.length?leftCells[leftCells.length-1]:"";
      let dim=model==="PALLET_STEP"?parseSpDimensionLabel(dimCell):parseWeightDimensionLabel(dimCell,previousWeightTo);
      if(!dim){
        if(nonEmpty(row).length===0)empty++;
        if(dataRows&&empty>=3)break;
        continue;
      }
      empty=0;
      const populated=zoneCols.filter(z=>num(row[z.col])!==null);
      if(!populated.length){
        if(dataRows)break;
        continue;
      }
  ...
  function inferNordicMatrixCountry(sheetName,rows){
    const sn=lower(sheetName),ctx=lower((rows||[]).slice(0,16).map(rowText).join(" | "));
    if(/horsens|taastrup|danmark|denmark/.test(sn+" "+ctx))return "DK";
    if(/katrineholm|sverige|sweden|svensk|från\s+pnr/.test(sn+" "+ctx))return "SE";
    return inferCountryFromSheet(sheetName)||inferCountryFromText(ctx);
  }

  function nordicCurrency(country,rows){
    const ctx=lower((rows||[]).slice(0,15).map(rowText).join(" | "));
    if(/\bsek\b/.test(ctx)||country==="SE")return "SEK";
    if(/\bdkk\b/.test(ctx)||country==="DK")return "DKK";
    if(/\beur\b|€/.test(ctx))return "EUR";
    return "EUR";
  }

  function nordicPalletQty(v){
    const t=text(v);
    if(/[½]/.test(t))return 0.5;
    const n=num(t);return n===null?null:n;
  }

  function nordicPostalRanges(country,expr){
    const len=POSTAL_LENGTH[country]||5;
    let raw=text(expr);
    if(!raw)return [];
    raw=raw.replace(/post\s*nr|postnr|pnr/gi," ").replace(/[+;\n\r]/g,",");
    const tokens=raw.match(/\d{1,5}\s*-\s*\d{1,5}|\d{1,5}/g)||[];
    const out=[];
    for(const tokenRaw of tokens){
      ...
      if(m){
        ...
        if(a.length===len&&b.length===len){
          out.push({from:a.padStart(len,"0"),to:b.padStart(len,"0")});
        }else{
          out.push({from:a.padEnd(len,"0"),to:b.padEnd(len,"9")});
        }
      }else if(/^\d{1,5}$/.test(token)){
        if(token.length>=len)out.push({from:token.slice(0,len),to:token.slice(0,len)});
        else out.push(postcodeRange(country,token));
      }
    }
    const seen=new Set();
    return out.filter(r=>{const k=r.from+"|"+r.to;if(seen.has(k))return false;seen.add(k);return true;});
  }

  function nordicSourceMeta(rows){
    const meta={};
    for(let r=0;r<Math.min(rows.length,12);r++){
      const row=rows[r]||[];
      for(let c=0;c<row.length;c++){
        const k=lower(row[c]),v=text(row[c+2]??row[c+1]??"");
        if(/firma|company/.test(k)&&v)meta.company=v;
        if(/dato|datum/.test(k)&&v)meta.date=v;
        if(/diesel/.test(k)&&v)meta.diesel=v;
        if(/vejafgifter|maut|toll/.test(k)&&v)meta.toll=v;
      }
    }
    return meta;
  }
  ...
  function detectNordicPriceModel(rows,priceCols,dataRows,sheetName){
    const ctx=lower((rows||[]).slice(0,16).map(rowText).join(" | ")+" "+sheetName);
    if(/\bftl\b|full\s*load/.test(ctx)&&dataRows.length<=2)return "FULL_LOAD";
    const ratios=[];
    for(const c of priceCols){
      const vals=dataRows.map(r=>num((rows[r]||[])[c.col])).filter(v=>v!==null&&v>0);
      if(vals.length>=3)ratios.push(vals[vals.length-1]/vals[0]);
    }
    const med=median(ratios);
    if(med!==null&&med<0.90)return "PER_PALLET";
    return "PALLET_STEP";
  }

  function parseNordicBidMatrix(sheetName,rows){
    let zoneRow=-1,axisRow=-1,palletCol=-1,ldmCol=-1;
    for(let i=0;i<Math.min(rows.length,30);i++){
      const row=rows[i]||[];
      const zoneCount=row.filter(v=>/^zone\s*\d+/i.test(text(v))).length;
      if(zoneCount<1)continue;
      const next=rows[i+1]||[],norm=next.map(v=>lower(v));
      const pc=norm.findIndex(v=>/antal\s*(?:pll|pall)|pallet/.test(v));
      const lc=norm.findIndex(v=>/^ldm$|lademeter/.test(v));
      if(pc>=0&&lc>=0){zoneRow=i;axisRow=i+1;palletCol=pc;ldmCol=lc;break;}
    }
    if(zoneRow<0)return null;

    const country=inferNordicMatrixCountry(sheetName,rows);
    if(!country)return null;
    const currency=nordicCurrency(country,rows),blockId=makeId("block");
    const zrow=rows[zoneRow]||[],arow=rows[axisRow]||[];

    /* Jede tatsächlich bepreiste physische Spalte ist eine eigene Tarifzone.
       Das ist absichtlich robuster als die teilweise nicht gepflegten Header
       "more zones…" bzw. doppelt vergebenen Zonennummern. */
    const rawCols=[];
    for(let c=Math.max(palletCol,ldmCol)+1;c<Math.max(zrow.length,arow.length);c++){
      const header=text(zrow[c]),region=text(arow[c]);
      const looksLikeZone=/^zone\s*\d+/i.test(header)||/more\s*zones/i.test(header);
      if(!looksLikeZone&&!region)continue;
      let hasPrice=false;
      for(let r=axisRow+1;r<rows.length;r++){
        if(num((rows[r]||[])[c])!==null){hasPrice=true;break;}
      }
      if(hasPrice)rawCols.push({col:c,header,region});
    }
    if(!rawCols.length)return null;

    const dataRows=[];
    for(let r=axisRow+1;r<rows.length;r++){
      const q=nordicPalletQty((rows[r]||[])[palletCol]);
      const ldm=num((rows[r]||[])[ldmCol]);
      const hasPrice=rawCols.some(c=>num((rows[r]||[])[c.col])!==null);
      if((q!==null||ldm!==null)&&hasPrice)dataRows.push(r);
    }
    if(!dataRows.length)return null;

    const model=detectNordicPriceModel(rows,rawCols,dataRows,sheetName);
    const sourceHeaders=rawCols.map(c=>c.header);
    const hasHeaderAnomalies=sourceHeaders.some(h=>/more\s*zones/i.test(h))||
      new Set(sourceHeaders.filter(h=>/^zone\s*\d+/i.test(h)).map(h=>lower(h))).size<
      sourceHeaders.filter(h=>/^zone\s*\d+/i.test(h)).length;

    const zones=[],rates=[];
    rawCols.forEach((col,idx)=>{
      const tariffNo=idx+1,zoneKey=`${country}-NORDIC-Z${tariffNo}`;
      const ranges=nordicPostalRanges(country,col.region);
      ranges.forEach((rg,ri)=>zones.push({
        id:makeId("zone"),blockId,zoneSet:sheetName,ruleType:"POSTCODE",
        originCountry:country,originPostcodeFrom:"",originPostcodeTo:"",
        destCountry:country,destPostcodeFrom:rg.from,destPostcodeTo:rg.to,
        distanceFromKm:"",distanceToKm:"",originName:sheetName,destinationName:"",
        zone:zoneKey,manualZoneNumber:tariffNo,manualOrder:tariffNo,priority:100,
        sourceRef:sheetName+"!R"+(axisRow+1)+"C"+(col.col+1)
      }));
      for(const r of dataRows){
        const price=num((rows[r]||[])[col.col]);if(price===null)continue;
        const pallets=nordicPalletQty((rows[r]||[])[palletCol]);
        const ldm=num((rows[r]||[])[ldmCol]);
        if(model==="FULL_LOAD"){
          rates.push({
            id:makeId("rate"),blockId,sheet:sheetName,sourceRow:r+1,manualOrder:r,
            rateModel:"FULL_LOAD",product:"FTL",subservice:sheetName,
            originCountry:country,destCountry:country,zone:zoneKey,relationName:"",
            chargeFrom:"",chargeTo:"",chargeLabel:"FTL",
            unit:`${currency}/SHIPMENT`,price:round(price,4),currency,priority:150,
            pallets:pallets,ldmEquivalent:ldm,sourceZoneHeader:col.header,sourceZoneRegion:col.region,
            sourceRef:sheetName+"!R"+(r+1)+"C"+(col.col+1)
          });
        }else{
          rates.push({
            id:makeId("rate"),blockId,sheet:sheetName,sourceRow:r+1,manualOrder:r,
            rateModel:model,product:"Palette / LDM",subservice:sheetName,
            originCountry:country,destCountry:country,zone:zoneKey,relationName:"",
            chargeFrom:pallets,chargeTo:pallets,chargeLabel:`${pallets===0.5?"½":text((rows[r]||[])[palletCol])} PLL · ${ldm!==null?String(round(ldm,2)).replace(".",","):"—"} LDM`,
            unit:model==="PER_PALLET"?`${currency}/PALLET`:`${currency}/SHIPMENT`,
            price:round(price,4),currency,priority:100,ldmEquivalent:ldm,
            sourceZoneHeader:col.header,sourceZoneRegion:col.region,
            sourceRef:sheetName+"!R"+(r+1)+"C"+(col.col+1)
          });
        }
      }
    ...
    const dz=dedupeZones(zones),meta=nordicSourceMeta(rows);
    const populatedZones=new Set(rates.map(r=>r.zone)).size;
    const modelLabel=model==="PER_PALLET"?"Preis je Palette":model==="FULL_LOAD"?"FTL":"Paletten-/LDM-Staffel";
    return {
      id:blockId,sheet:sheetName,headerRow:zoneRow+1,model,modelLabel,
      confidence:hasHeaderAnomalies?0.90:0.99,status:hasHeaderAnomalies?"REVIEW":"AUTO_CANDIDATE",
      rates,zones:dz,sourceMeta:meta,zoneMode:"NORDIC_INLINE_POSTCODES",
      sourceZoneHeaders:rawCols.map((c,i)=>({column:c.col+1,sourceHeader:c.header,postcode:c.region,normalizedZone:i+1})),
      summary:`${rates.length} Preise · ${populatedZones} Tarifzonen · ${dz.length} PLZ-Regeln`,
      context:`Nordische Angebotsmatrix: ${modelLabel} · ${country} · ${currency}${hasHeaderAnomalies?" · Zonenheader bitte prüfen":""}`
    };
  }
  ...
  function parseExplicitClassicGrid(sheetName,rows){
    ...
    for(let i=0;i<rows.length;i++){
      ...
      if(!/(plz\s*\/\s*zone|plz\s*\/\s*ldm|plz\s*\/\s*(?:kg|tonnen)|plz\s*\/\s*palette|plz\s*\/\s*sp)/.test(rt))continue;
  ...
  function analyzeSheet(sheetName,rows){
    const blocks=[];
    const nordic=parseNordicBidMatrix(sheetName,rows);
    if(nordic)blocks.push(nordic);
    const efCustom=parseEversFrankCustomZoneMatrix(sheetName,rows);
    if(efCustom&&!blocks.some(x=>overlap(x,efCustom)))blocks.push(efCustom);
    const efSp=parseEversFrankSpMatrix(sheetName,rows);
    if(efSp&&!blocks.some(x=>overlap(x,efSp)))blocks.push(efSp);
    const efWeight=parseEversFrankWeightMatrix(sheetName,rows);
    if(efWeight&&!blocks.some(x=>overlap(x,efWeight)))blocks.push(efWeight);
    const trocellenGrid=parseTrocellenLdmTransportGrid(sheetName,rows);
    if(trocellenGrid&&!blocks.some(x=>overlap(x,trocellenGrid)))blocks.push(trocellenGrid);
  ...
  function destinationCountryFromPmlSheet(sheetName,rows){
    const sn=text(sheetName).toUpperCase();
    let m=sn.match(/\bDE\s*-\s*([A-Z]{2})\b/);
    if(m)return m[1];
    const title=(rows||[]).slice(0,8).map(rowText).join(" | ");
    m=text(title).toUpperCase().match(/\bANGEBOTSBLATT\s+([A-Z]{2})\b/);
    if(m)return m[1];
    if(/TKL.*MELDORF|PREETZ.*STP/i.test(sheetName))return "DE";
    return inferCountryFromText(title)||inferCountryFromSheet(sheetName);
  }

  function pmlZoneSheetForCountry(sheets,country){
    const wanted=`ZONEN DE-${country}`.toUpperCase();
    return (sheets||[]).find(sh=>text(sh.name).toUpperCase()===wanted)||null;
  }

  function pmlPostalRange(country,fromValue,toValue){
    const len=POSTAL_LENGTH[country]||5;
    const fromRaw=text(fromValue).replace(/\*/g,"").trim();
    const toRaw=text(toValue).replace(/\*/g,"").trim();

    if(toRaw && /^\d+$/.test(fromRaw) && /^\d+$/.test(toRaw)){
      return {
        from:fromRaw.padStart(len,"0").slice(-len),
        to:toRaw.padStart(len,"0").slice(-len)
      };
    }

    const compact=fromRaw.replace(/\s+/g,"");
    let m=compact.match(/^(\d{1,5})-(\d{1,5})$/);
    if(m){
      const a=m[1],b=m[2];
      /* In den internationalen PML-Zonenblättern sind z. B. "10-36"
         zweistellige PLZ-Präfixe und keine vollständigen Postleitzahlen. */
      if(a.length<len && b.length<len){
        return {from:a.padEnd(len,"0"),to:b.padEnd(len,"9")};
      }
      return {from:a.padStart(len,"0").slice(-len),to:b.padStart(len,"0").slice(-len)};
    }
    if(/^\d{1,5}$/.test(compact))return postcodeRange(country,compact);
    return null;
  }

  function parsePmlCompanionZones(sheet,country,blockId){
    if(!sheet)return {zones:[],onRequest:[],headerRow:-1};
    const rows=sheet.rows||[];let header=-1,plzCol=0,toCol=-1,zoneCol=-1;
    for(let i=0;i<Math.min(rows.length,30);i++){
      const row=(rows[i]||[]).map(v=>lower(v));
      const z=row.findIndex(v=>v==="zone");
      const p=row.findIndex(v=>/plz/.test(v));
      if(z>=0&&p>=0){
        header=i;zoneCol=z;plzCol=p;
        const bt=row.findIndex((v,c)=>c!==p&&/bis\s*plz|plz\s*bis/.test(v));
        toCol=bt>=0?bt:(z>=2?1:-1);
        break;
      }
    }
    if(header<0)return {zones:[],onRequest:[],headerRow:-1};

    const zones=[],onRequest=[];
    for(let r=header+1;r<rows.length;r++){
      const row=rows[r]||[],fromV=row[plzCol],toV=toCol>=0?row[toCol]:"",zoneV=row[zoneCol];
      if(!text(fromV)&&!text(zoneV))continue;
      const zn=num(zoneV);
      const rg=pmlPostalRange(country,fromV,toV);
      if(!rg)continue;
      if(zn===null||!Number.isFinite(zn)){
        onRequest.push({country,from:rg.from,to:rg.to,label:text(zoneV)||"auf Anfrage",sourceRef:sheet.name+"!R"+(r+1)});
        continue;
      }
      const n=Math.trunc(zn);
      zones.push({
        id:makeId("zone"),blockId,zoneSet:sheet.name,ruleType:"POSTCODE",
        originCountry:"DE",originPostcodeFrom:"",originPostcodeTo:"",
        destCountry:country,destPostcodeFrom:rg.from,destPostcodeTo:rg.to,
        distanceFromKm:"",distanceToKm:"",originName:"25704 Meldorf / Preetz",destinationName:"",
        zone:`${country}-Z${n}`,manualZoneNumber:n,manualOrder:n,priority:100,
        sourceRef:sheet.name+"!R"+(r+1)
      });
    }
    return {zones:dedupeZones(zones),onRequest,headerRow:header+1};
  }

  function parsePmlDirectZoneHeader(country,label,blockId,sheetName,col){
    const t=text(label).toUpperCase();
    let m=t.match(/^([A-Z]{2})\s*-\s*(\d{1,5})X+$/);
    if(!m)m=t.match(/^([A-Z]{2})\s*[- ]\s*(\d{1,5})\*+$/);
    if(!m)return null;
    const c=m[1]||country,prefix=m[2],rg=postcodeRange(c,prefix);
    return {
      id:makeId("zone"),blockId,zoneSet:sheetName,ruleType:"POSTCODE",
      originCountry:"DE",originPostcodeFrom:"",originPostcodeTo:"",
      destCountry:c,destPostcodeFrom:rg.from,destPostcodeTo:rg.to,
      distanceFromKm:"",distanceToKm:"",originName:"25704 Meldorf / Preetz",destinationName:"",
      zone:`${c}-DIRECT-${prefix}`,manualZoneNumber:col,manualOrder:col,priority:100,
      sourceRef:sheetName+"!HEADER-C"+(col+1)
    };
  }

  function parsePmlStpRateSheet(sheet,sheets){
    const rows=sheet.rows||[];let header=-1,qtyCol=-1,unitCol=-1,zoneCols=[];
    for(let i=0;i<Math.min(rows.length,20);i++){
      const row=rows[i]||[],norm=row.map(v=>lower(v));
      const q=norm.findIndex(v=>v==="menge");
      const u=norm.findIndex(v=>/^einheit/.test(v));
      if(q<0||u<0)continue;
      const found=[];
      for(let c=0;c<row.length;c++){
        const t=text(row[c]);
        const zm=t.match(/^zone\s*(\d{1,3})$/i);
        if(zm)found.push({col:c,zoneNumber:Number(zm[1]),header:t,type:"ZONE"});
        else if(/^[A-Z]{2}\s*-\s*\d{1,5}X+$/i.test(t))found.push({col:c,zoneNumber:found.length+1,header:t,type:"DIRECT"});
      }
      if(found.length>=1){header=i;qtyCol=q;unitCol=u;zoneCols=found;break;}
    }
    if(header<0)return null;

    const country=destinationCountryFromPmlSheet(sheet.name,rows);
    if(!country)return null;
    const blockId=makeId("block");

    /* Nur tatsächlich bepreiste Zonenspalten übernehmen. */
    zoneCols=zoneCols.filter(z=>{
      for(let r=header+1;r<rows.length;r++){
        const q=text((rows[r]||[])[qtyCol]),unit=lower((rows[r]||[])[unitCol]);
        const p=num((rows[r]||[])[z.col]);
        if(p!==null&&(/^(\d+(?:[.,]\d+)?)$/.test(q)&&/stp|stellplatz/.test(unit)||/^m\/m\b/i.test(q)))return true;
      }
      return false;
    });
    if(!zoneCols.length)return null;

    const companion=pmlZoneSheetForCountry(sheets,country);
    const companionParsed=parsePmlCompanionZones(companion,country,blockId);
    let zones=companionParsed.zones.slice();

    /* Direkte Regionen wie "ES - 28xxx" brauchen kein separates Zonenblatt. */
    zoneCols.forEach((zc,i)=>{
      if(zc.type==="DIRECT"){
        const z=parsePmlDirectZoneHeader(country,zc.header,blockId,sheet.name,i+1);
        if(z){
          zc.zoneKey=z.zone;zc.zoneNumber=i+1;zones.push(z);
        }
      }else zc.zoneKey=`${country}-Z${zc.zoneNumber}`;
    });

    const rates=[],minimumCharges={},requestRows=[];
    for(let r=header+1;r<rows.length;r++){
      const row=rows[r]||[],qtyText=text(row[qtyCol]),unit=lower(row[unitCol]);
      const isMin=/^m\/m\b/i.test(qtyText);
      const qty=num(qtyText);
      const numericPrices=zoneCols.map(z=>num(row[z.col])).filter(v=>v!==null);
      const onRequest=zoneCols.some(z=>/anfrage/i.test(text(row[z.col])));
      if(isMin){
        zoneCols.forEach(z=>{const price=num(row[z.col]);if(price!==null)minimumCharges[z.zoneKey]=round(price,4);});
        continue;
      }
      if(onRequest&&numericPrices.length===0){
        requestRows.push({label:qtyText,unit:text(row[unitCol]),sourceRef:sheet.name+"!R"+(r+1)});
        continue;
      }
      if(qty===null||!/stp|stellplatz/.test(unit)||!numericPrices.length)continue;

      for(const z of zoneCols){
        const price=num(row[z.col]);if(price===null)continue;
        rates.push({
          id:makeId("rate"),blockId,sheet:sheet.name,sourceRow:r+1,manualOrder:r,
          rateModel:"PER_PALLET",product:"Stellplatz",subservice:country==="DE"?"DE Stückgut / Teilpartie":`DE-${country} STP`,
          originCountry:"DE",destCountry:country,zone:z.zoneKey,relationName:"",
          chargeFrom:qty,chargeTo:qty,chargeLabel:`${text(qtyText)} STP`,
          unit:"EUR/PALLET",price:round(price,4),currency:"EUR",priority:100,
          minimumPrice:minimumCharges[z.zoneKey]??null,
          sourceRef:sheet.name+"!R"+(r+1)+"C"+(z.col+1)
        });
      }
    }
    if(rates.length<2)return null;

    /* Mindestpreise wurden vor den STP-Zeilen gelesen; auf Rate Records spiegeln. */
    rates.forEach(rate=>{if(minimumCharges[rate.zone]!=null)rate.minimumPrice=minimumCharges[rate.zone];});

    const usedZoneKeys=new Set(rates.map(r=>r.zone));
    zones=dedupeZones(zones.filter(z=>usedZoneKeys.has(z.zone)));
    const tariffZoneCount=new Set(rates.map(r=>r.zone)).size;
    const confidence=zones.length||zoneCols.every(z=>z.type==="DIRECT")?0.99:0.82;

    return {
      id:blockId,sheet:sheet.name,headerRow:header+1,model:"PER_PALLET",
      modelLabel:"Stellplatzpreis je STP",confidence,status:confidence>=0.90?"AUTO_CANDIDATE":"REVIEW",
      rates,zones,minimumCharges,onRequestRules:companionParsed.onRequest,requestRows,
      zoneMode:"PML_COMPANION_ZONE_SHEET",
      summary:`${rates.length} Preise · ${tariffZoneCount} Tarifzonen · ${zones.length} PLZ-Regeln`,
      context:`PML-Angebotsblatt: Mengenstaffel je Stellplatz · Zonen aus ${companion?.name||"Tarifkopf"}`
    };
  }

  function parsePmlStpWorkbook(sheets){
    const blocks=[],consumed=new Set();
    for(const sheet of sheets||[]){
      const n=text(sheet.name);
      if(/^Zonen\s+/i.test(n))continue;
      const likely=/\bSTP\b/i.test(n)||/^DE-FR$/i.test(n);
      if(!likely)continue;
      const block=parsePmlStpRateSheet(sheet,sheets);
      if(!block)continue;
      blocks.push(block);consumed.add(sheet.name);
      const country=destinationCountryFromPmlSheet(sheet.name,sheet.rows);
      const zs=pmlZoneSheetForCountry(sheets,country);if(zs)consumed.add(zs.name);
    }
  ...
  async function analyzeFile(file){
    ...
    const pml=parsePmlStpWorkbook(sheets);
    blocks.push(...pml.blocks);
    for(const sheet of sheets){
      if(pml.consumed.has(sheet.name))continue;
      try{
  ...
  function installModal(){
    ...
    document.body.insertAdjacentHTML("beforeend",`
      <div class="modal-backdrop" id="gpkTariffAutoImport" hidden>
        <section class="location-modal tariff-auto-import-modal" role="dialog" aria-modal="true">
          ...
          <div class="auto-import-topbar import-master-row import-master-row-inline">
            <div class="field import-master-provider"><label>Dienstleister</label><div class="import-provider-select-row"><select id="gpkTariffAutoProvider" required></select><button class="secondary compact-button import-square-button" id="gpkTariffRefreshProviders" type="button" title="Dienstleisterliste aktualisieren">↻</button><button class="secondary compact-button import-create-provider-btn" id="gpkTariffCreateProvider" type="button">+ Dienstleister anlegen</button><div class="import-inline-version"><label for="gpkTariffImportVersion">Version</label><input id="gpkTariffImportVersion" type="text" placeholder="z. B. V1"></div></div><div id="gpkTariffProviderError" class="field-error" hidden>Bitte zuerst einen Dienstleister auswählen.</div></div>
          </div>
          <div class="auto-import-body">
            <div class="auto-import-blocks"><div class="import-section-head compact-import-head"><div><h3>Importauswahl</h3></div></div><div id="gpkTariffAutoSummary" class="auto-import-summary-compact"></div><div class="compact-country-panel country-before-blocks"><span class="auto-filter-label">Land</span><div id="gpkTariffCountryTabs" class="auto-filter-tabs compact-country-tabs"></div></div><div id="gpkTariffAutoBlocks" class="auto-import-block-list"></div></div>
            <div class="auto-import-preview">
              <div class="import-section-head"><div><h3>Ratenblatt prüfen</h3><p id="gpkTariffAutoPreviewMeta"></p></div><div class="preview-toggle"><button class="secondary compact-button" id="gpkTariffEditToggle" type="button">Bearbeiten</button><button class="secondary compact-button" id="gpkTariffAddRateRow" type="button" hidden>+ Zeile am Ende</button><button class="secondary compact-button" id="gpkTariffAddArea" type="button" hidden>+ Bereich</button><button class="secondary compact-button" id="gpkTariffAddZone" type="button" hidden>+ Zone</button><button class="secondary compact-button" id="gpkTariffSortZones" type="button" hidden title="Zonen numerisch sortieren">Zonen 1→n</button><button class="secondary compact-button" id="gpkTariffBenchmarkMetaToggle" type="button" hidden>Benchmark-Felder</button><button class="preview-toggle-btn active" data-auto-preview="rates" type="button">Ratenblatt</button><button class="preview-toggle-btn" data-auto-preview="zones" type="button">Zonenblatt</button></div></div>
    ...
    document.getElementById("gpkTariffSortZones").addEventListener("click",()=>sortZonesByNumber());
    ...
    document.getElementById("gpkTariffRefreshProviders").addEventListener("click",()=>{const current=text(document.getElementById("gpkTariffAutoProvider").value);renderProviderList(current);clearProviderError();updateConfirm();persistDraft();});
    document.getElementById("gpkTariffAutoProvider").addEventListener("change",()=>{clearProviderError();updateConfirm();persistDraft();});
  ...
  const state={file:null,analysis:null,selected:new Set(),preview:"rates",country:"",countryFilter:new Set(),blockId:"",editing:false,_matrix:null,editBatchId:"",metaOpen:false};
  function draftPayload(){
    return {fileName:state.file?.name||state.analysis?.fileName||"Tarif.xlsx",analysis:state.analysis,selected:[...state.selected],preview:state.preview,country:state.country,countryFilter:[...state.countryFilter],blockId:state.blockId,provider:text(document.getElementById("gpkTariffAutoProvider")?.value||""),editBatchId:state.editBatchId||"",version:text(document.getElementById("gpkTariffImportVersion")?.value||""),savedAt:new Date().toISOString()};
  }
  ...
  function restoreDraft(){
    ...
    installModal();state.file={name:d.fileName||d.analysis.fileName||"Tarif.xlsx"};state.analysis=d.analysis;state.selected=new Set(d.selected||[]);state.preview=d.preview||"rates";state.country=d.country||"";state.countryFilter=new Set(d.countryFilter||[]);state.blockId=d.blockId||"";state.editing=false;state._matrix=null;state.editBatchId=d.editBatchId||"";state.metaOpen=false;
    document.getElementById("gpkTariffAutoFile").textContent=state.file.name;document.getElementById("gpkTariffAutoImport").hidden=false;document.body.classList.add("modal-open");renderProviderList(d.provider||state.analysis.provider||"");clearProviderError();const draftVersion=document.getElementById("gpkTariffImportVersion");if(draftVersion)draftVersion.value=d.version||"EXT";renderSummary();renderBlocks();ensurePreviewSelection();renderPreview();syncEditControls();updateConfirm();return true;
  }
  ...
  function clearProviderError(){const sel=document.getElementById("gpkTariffAutoProvider"),e=document.getElementById("gpkTariffProviderError");sel?.classList.remove("input-error");if(e){e.hidden=true;e.textContent="Bitte zuerst einen Dienstleister auswählen.";e.classList.remove("success-message");}}
  function selectedProviderRecord(){
    const sel=document.getElementById("gpkTariffAutoProvider");if(!sel)return null;
    const value=text(sel.value),id=text(sel.selectedOptions?.[0]?.dataset?.providerId||"");
    const records=providerMasterRecords();
    return records.find(p=>id&&text(p.id)===id)||records.find(p=>text(p.name).toLowerCase()===value.toLowerCase())||null;
  }
  function renderSummary(){
    ...
    const rates=a.blocks.flatMap(b=>b.rates||[]).filter(r=>Number.isFinite(Number(r.price)));
    const zones=dedupeZones(a.blocks.flatMap(b=>b.zones||[]));
    const tariffZones=countTariffZones(zones);
    const countries=[...new Set(a.blocks.flatMap(blockCountries).filter(c=>c&&c!=="ALL"))].sort();
    const el=document.getElementById("gpkTariffAutoSummary");if(!el)return;
    el.innerHTML=`<strong>${a.blocks.length} Bereiche</strong><span>${rates.length.toLocaleString("de-DE")} Preise</span><span>${tariffZones.toLocaleString("de-DE")} Tarifzonen</span><span>${zones.length.toLocaleString("de-DE")} PLZ-Regeln</span><span>${countries.length} Länder</span>${review?`<span class="review-text">${review} Review</span>`:""}`;
  }
  function renderBlocks(){
    ...
    const selectedCountries=[...state.countryFilter];
    const visible=selectedCountries.length
      ? state.analysis.blocks.filter(b=>{const bc=blockCountries(b);return selectedCountries.some(c=>c==="ALL"?bc.includes("ALL"):bc.includes(c)||bc.includes("ALL"));})
      : state.analysis.blocks;
    wrap.innerHTML=visible.map(b=>`<div class="auto-block-card compact-block-card ${b.id===state.blockId?"active-area":""} ${b.status==="REVIEW"?"review":""}" data-area-card="${esc(b.id)}"><label class="block-select-label"><input type="checkbox" data-auto-block="${esc(b.id)}" ${state.selected.has(b.id)?"checked":""}><span class="auto-block-main"><span class="auto-block-title"><strong>${esc(blockAreaLabel(b))}</strong><span class="confidence-pill ${b.status==="REVIEW"?"review":"auto"}">${Math.round(b.confidence*100)}%</span></span></span></label><span class="block-card-actions"><button type="button" data-block-rename="${esc(b.id)}" title="Bereich umbenennen">✎</button><button type="button" data-block-delete="${esc(b.id)}" title="Bereich löschen">×</button></span></div>`).join("")||`<div class="empty-state auto-import-empty"><strong>Für dieses Land ist kein Bereich vorhanden.</strong></div>`;
    wrap.querySelectorAll("[data-auto-block]").forEach(cb=>cb.addEventListener("change",e=>{e.stopPropagation();if(cb.checked)state.selected.add(cb.dataset.autoBlock);else state.selected.delete(cb.dataset.autoBlock);ensurePreviewSelection();renderBlocks();renderPreviewFilters();renderPreview();updateConfirm();persistDraft();}));
    wrap.querySelectorAll("[data-area-card]").forEach(card=>card.addEventListener("click",e=>{if(e.target.closest("button,input,label"))return;const id=card.dataset.areaCard;if(!state.selected.has(id))state.selected.add(id);state.blockId=id;const b=state.analysis.blocks.find(x=>x.id===id);const cs=blockCountries(b);if(!cs.includes(state.country)){state.country=cs[0]||state.country;if(state.country)state.countryFilter.add(state.country);}renderBlocks();renderPreviewFilters();renderPreview();updateConfirm();persistDraft();}));
    wrap.querySelectorAll("[data-block-rename]").forEach(btn=>btn.addEventListener("click",e=>{e.stopPropagation();renameBlock(btn.dataset.blockRename);}));
    wrap.querySelectorAll("[data-block-delete]").forEach(btn=>btn.addEventListener("click",e=>{e.stopPropagation();deleteBlock(btn.dataset.blockDelete);}));
  }
  function renameBlock(id){const b=state.analysis?.blocks.find(x=>x.id===id);if(!b)return;const name=window.prompt("Name des Tarifbereichs:",blockAreaLabel(b));if(name===null||!text(name))return;b.displayName=text(name);if(b.benchmarkMeta)b.benchmarkMeta.subservice=text(name);Object.values(b.benchmarkMetaByCountry||{}).forEach(m=>m.subservice=text(name));persistDraft();renderBlocks();renderPreview();}
  function deleteBlock(id){const b=state.analysis?.blocks.find(x=>x.id===id);if(!b)return;if(!window.confirm(`Tarifbereich „${blockAreaLabel(b)}“ wirklich löschen?`))return;state.analysis.blocks=state.analysis.blocks.filter(x=>x.id!==id);state.selected.delete(id);if(state.blockId===id)state.blockId="";const remainingCountries=allImportCountries();[...state.countryFilter].forEach(c=>{if(!remainingCountries.includes(c))state.countryFilter.delete(c);});renderSummary();ensurePreviewSelection();renderBlocks();renderPreview();updateConfirm();persistDraft();}
  ...
  function allImportCountries(){
    return state.analysis?[...new Set(state.analysis.blocks.flatMap(blockCountries).filter(Boolean))].sort((a,b)=>a==="ALL"?1:b==="ALL"?-1:a.localeCompare(b)):[];
  }
  ...
  function blockAreaLabel(block){
    if(text(block?.displayName))return text(block.displayName);
  ...
  function ensurePreviewSelection(){
    const countries=allImportCountries();
    if(!countries.includes(state.country))state.country=[...state.countryFilter].find(c=>countries.includes(c))||countries[0]||"";
    [...state.countryFilter].forEach(c=>{if(!countries.includes(c))state.countryFilter.delete(c);});
    const cbs=countryBlocks(state.country);
    if(!cbs.some(b=>b.id===state.blockId)){
      const selected=selectedBlocks();
      state.blockId=cbs[0]?.id||selected[0]?.id||"";
    }
  ...
  function renderPreviewFilters(){
    ...
    const countries=allImportCountries();
    cWrap.innerHTML=countries.map(c=>`<button type="button" class="auto-filter-tab multi-country-tab ${state.countryFilter.has(c)?"selected":""} ${state.countryFilter.size&&c===state.country?"active-preview":""}" data-auto-country="${esc(c)}" title="Mehrfachauswahl möglich">${esc(c==="ALL"?"Kein Land":c)}</button>`).join("")||`<span class="auto-filter-empty">—</span>`;
    cWrap.querySelectorAll("[data-auto-country]").forEach(btn=>btn.addEventListener("click",()=>{
      const c=btn.dataset.autoCountry;
      if(state.countryFilter.has(c)){
        state.countryFilter.delete(c);
        if(state.country===c)state.country=[...state.countryFilter][0]||c;
      }else{
        state.countryFilter.add(c);state.country=c;
      }
      const cbs=countryBlocks(state.country);
      if(state.countryFilter.size&&!cbs.some(b=>b.id===state.blockId))state.blockId=cbs[0]?.id||selectedBlocks()[0]?.id||"";
      renderBlocks();renderPreviewFilters();renderPreview();syncEditControls();persistDraft();
    }));
  ...
  function buildZoneColumns(block,country,rates,zones){
    ...
    let cols=[...map.values()].sort(zoneSort);
    if(block?.zoneMode==="TARIFF_ZONE_ASSIGNMENT"){
      const hasExplicitOrder=cols.some(col=>(col.rules||[]).some(z=>Number.isFinite(Number(z.manualOrder))));
      if(!hasExplicitOrder){
        cols=cols.sort((a,b)=>{
          const an=(a.rules||[]).map(z=>Number(z.manualZoneNumber)).find(Number.isFinite);
          const bn=(b.rules||[]).map(z=>Number(z.manualZoneNumber)).find(Number.isFinite);
    ...
    cols.forEach((col,i)=>{
      ...
      const assignment=(block?.zoneMode==="TARIFF_ZONE_ASSIGNMENT")?zoneAssignmentText(block,col,country):"";
      col.region=manualRegion||assignment||(labels.length<=2?labels.join(" / "):`${labels[0]} +${labels.length-1}`);
  ...
  function syncEditControls(){
    const toggle=document.getElementById("gpkTariffEditToggle"),add=document.getElementById("gpkTariffAddZone"),sortZones=document.getElementById("gpkTariffSortZones"),addRow=document.getElementById("gpkTariffAddRateRow"),addArea=document.getElementById("gpkTariffAddArea"),meta=document.getElementById("gpkTariffBenchmarkMeta"),metaToggle=document.getElementById("gpkTariffBenchmarkMetaToggle");
    if(toggle){toggle.textContent=state.editing?"Bearbeiten beenden":"Bearbeiten";toggle.classList.toggle("active",state.editing);}
    if(add)add.hidden=!state.editing;if(sortZones)sortZones.hidden=!state.editing;if(addRow)addRow.hidden=!state.editing;if(addArea)addArea.hidden=!state.editing;
  ...
  function addManualArea(){
    if(!state.analysis)return;const current=activeBlock();const name=window.prompt("Name des neuen Tarifbereichs:","Neuer Bereich");if(name===null||!text(name))return;const id=makeId("block");const baseMeta=current?benchmarkMetaFor(current,"ALL"):{};const block={id,sheet:text(name),displayName:text(name),headerRow:0,model:current?.model||"LDM_STEP",modelLabel:text(name),confidence:1,status:"AUTO_CANDIDATE",rates:[],zones:[],summary:"manuell angelegt",context:"Manueller Tarifbereich · noch keinem Land zugeordnet",benchmarkMetaByCountry:{ALL:{...baseMeta,destCountry:"",subservice:text(name)}},benchmarkMeta:{...baseMeta,destCountry:"",subservice:text(name)}};state.analysis.blocks.push(block);state.selected.add(id);state.blockId=id;state.country="ALL";state.countryFilter.add("ALL");renderSummary();renderPreviewFilters();renderBlocks();renderPreview();persistDraft();
  }
  ...
  function sortZonesByNumber(){
    const block=activeBlock();if(!block)return;
    const cols=currentZoneColumns(block,state.country).slice().sort((a,b)=>{
      const an=Number(a.zoneNumber),bn=Number(b.zoneNumber);
      if(Number.isFinite(an)&&Number.isFinite(bn)&&an!==bn)return an-bn;
      if(Number.isFinite(an)!==Number.isFinite(bn))return Number.isFinite(an)?-1:1;
      return text(a.region).localeCompare(text(b.region),undefined,{numeric:true});
    });
    setZoneOrder(cols);persistDraft();renderPreview();
  }
  ...
  function bindRateEditor(block,country){
    ...
    document.querySelectorAll("[data-zone-col-move]").forEach(btn=>btn.addEventListener("click",()=>{const [ci,dir]=btn.dataset.zoneColMove.split(":").map(Number);moveZoneColumn(m.cols[ci],dir);}));
  }
  ...
  function renderRateMatrix(block,country){
    ...
    document.getElementById("gpkTariffAutoPreviewMeta").textContent=`${country==="ALL"?'Allgemein':country} · ${blockAreaLabel(block)} · ${rates.length.toLocaleString('de-DE')} Preiszellen · ${cols.length} Tarifzonen · ${zones.length.toLocaleString('de-DE')} PLZ-Regeln`;
    ...
    head.innerHTML=`<tr>${leftHead}${cols.map((c,ci)=>`<th class="matrix-zone-head" title="${esc((c.rules||[]).map(z=>[z.destCountry,z.destPostcodeFrom,z.destPostcodeTo,z.destinationName].filter(Boolean).join(' ')).join(' · '))}">${state.editing?`<div class="matrix-zone-head-actions"><button type="button" data-zone-col-move="${ci}:-1" title="Zone nach links">←</button><button type="button" data-zone-col-move="${ci}:1" title="Zone nach rechts">→</button></div><input class="matrix-head-input matrix-region-input" data-matrix-zone-code="${ci}" value="${esc(c.region)}" aria-label="PLZ-Zuordnung"><label class="matrix-zone-number-wrap">Zone <input type="number" min="1" max="999" class="matrix-head-input matrix-zone-number-input" data-matrix-zone-number="${ci}" value="${esc(c.zoneNumber)}" aria-label="Zonennummer"></label>`:`<span class="matrix-region">${esc(c.region)}</span><span class="matrix-zone">Zone ${esc(c.zoneNumber)}</span>`}</th>`).join('')}</tr>`;
  ...
  function renderZoneSheet(block,country){
    const zones=dedupeZones((block.zones||[]).filter(z=>country==="ALL"?!text(z.destCountry):!text(z.destCountry)||text(z.destCountry).toUpperCase()===country));
    const cols=buildZoneColumns(block,country,[],zones),index=new Map(cols.map((c,i)=>[c.key,i+1]));
    const tariffZoneMode=block.zoneMode==="TARIFF_ZONE_ASSIGNMENT";
    document.getElementById("gpkTariffAutoPreviewMeta").textContent=tariffZoneMode
      ? `${country==="ALL"?"Allgemein":country} · ${blockAreaLabel(block)} · ${cols.length} Tarifzonen · ${zones.length.toLocaleString("de-DE")} PLZ-Regeln`
      : `${country==="ALL"?"Allgemein":country} · ${blockAreaLabel(block)} · ${zones.length.toLocaleString("de-DE")} Zonenregeln`;
    document.getElementById("gpkTariffPreviewContext").innerHTML=`<span><strong>${esc(block.modelLabel||block.model)}</strong>${esc(block.sheet)} · Zonenauflösung${state.editing?" · Bearbeitungsmodus":""}</span>`;
    const head=document.getElementById("gpkTariffAutoPreviewHead"),body=document.getElementById("gpkTariffAutoPreviewRows");

    if(tariffZoneMode){
      head.innerHTML=state.editing
        ? "<tr><th>↕</th><th>Zone</th><th>Land</th><th>PLZ-Zuordnung</th><th>PLZ-Regeln</th></tr>"
        : "<tr><th>Zone</th><th>Land</th><th>PLZ-Zuordnung</th><th>PLZ-Regeln</th></tr>";
      body.innerHTML=cols.map((col,ci)=>{
        const z=col.rules?.[0]||{},number=col.zoneNumber||index.get(col.key)||ci+1;
        const assignment=zoneAssignmentText(block,col,country);
        if(state.editing)return `<tr>
          <td class="zone-row-actions"><button type="button" data-zone-move="${ci}:-1" title="nach oben">↑</button><button type="button" data-zone-move="${ci}:1" title="nach unten">↓</button></td>
          <td><label class="zone-number-cell">Zone <input type="number" min="1" max="999" class="zone-edit-input zone-number-input" data-zone-edit="${ci}:number" value="${esc(number)}"></label></td>
          <td><input class="zone-edit-input zone-country-input" data-zone-assignment-country="${ci}" value="${esc(z.destCountry||country||"DE")}"></td>
          <td><input class="zone-edit-input zone-assignment-input" data-zone-assignment="${ci}" value="${esc(assignment)}" placeholder="z. B. 24, 25 oder 20 - 23"></td>
          <td><strong>${(col.rules||[]).length}</strong><small> PLZ-Regeln</small></td>
        </tr>`;
        return `<tr><td><strong>Zone ${esc(number)}</strong></td><td>${esc(z.destCountry||country||"—")}</td><td><strong>${esc(assignment||"—")}</strong></td><td>${(col.rules||[]).length} PLZ-Regeln</td></tr>`;
      }).join("")||`<tr><td colspan="${state.editing?5:4}" class="empty-state">Keine Tarifzonen für diese Auswahl.</td></tr>`;

      if(state.editing){
        body.querySelectorAll("[data-zone-assignment]").forEach(el=>el.addEventListener("change",()=>{
          const ci=Number(el.dataset.zoneAssignment),col=cols[ci];
          const cInput=body.querySelector(`[data-zone-assignment-country="${ci}"]`);
          rebuildZoneAssignment(block,col,text(cInput?.value||country||"DE").toUpperCase(),el.value);
          persistDraft();renderSummary();renderPreview();
        }));
        body.querySelectorAll("[data-zone-assignment-country]").forEach(el=>el.addEventListener("change",()=>{
          const ci=Number(el.dataset.zoneAssignmentCountry),col=cols[ci];
          const aInput=body.querySelector(`[data-zone-assignment="${ci}"]`);
          rebuildZoneAssignment(block,col,text(el.value||country||"DE").toUpperCase(),aInput?.value||"");
          persistDraft();renderSummary();renderPreview();
        }));
        body.querySelectorAll("[data-zone-move]").forEach(btn=>btn.addEventListener("click",()=>{const [ci,dir]=btn.dataset.zoneMove.split(":").map(Number);moveZoneColumn(cols[ci],dir);}));
        body.querySelectorAll("[data-zone-edit]").forEach(el=>el.addEventListener("change",()=>{
          const [ci,field]=el.dataset.zoneEdit.split(":");const col=cols[Number(ci)];
          if(field==="number")updateZoneNumber(block,col,country,text(el.value));
          persistDraft();renderSummary();renderPreview();
        }));
      }
      return;
  ...
  function confirmImport(){
    ...
    const providerRecord=selectedProviderRecord();
    const provider=providerRecord?.name||text(providerSelect?.value);
    if(!provider||!providerRecord){
      const e=document.getElementById("gpkTariffProviderError");if(e){e.textContent=provider?"Dienstleister nicht in den Stammdaten gefunden. Bitte Liste aktualisieren oder Dienstleister anlegen.":"Bitte zuerst einen Dienstleister auswählen.";e.hidden=false;}
      providerSelect?.classList.add("input-error");providerSelect?.focus();
      return;
    }
    clearProviderError();
    if(!state.analysis)return;
    const blocks=selectedBlocks();if(!blocks.length){showImportMessage("Bitte mindestens einen Tarifbereich auswählen.");return;}
    const importVersion=text(document.getElementById("gpkTariffImportVersion")?.value||"EXT")||"EXT";
    ...
    try{
      ...
      refreshPageStatus();
      if(window.rateToast){rateToast.textContent=`Import erfolgreich: ${newRates.length.toLocaleString("de-DE")} Preise · ${countTariffZones(newZones).toLocaleString("de-DE")} Tarifzonen · ${newZones.length.toLocaleString("de-DE")} PLZ-Regeln.`;rateToast.hidden=false;setTimeout(()=>rateToast.hidden=true,3600);}
    }catch(err){
  ...
  function filteredImports(imports){
    ...
    return imports.filter(x=>{
      ...
      const mode=text(document.getElementById("rateSort")?.value||"newest");
      if(mode==="oldest")return new Date(a.createdAt||0)-new Date(b.createdAt||0);
      if(mode==="provider")return text(a.provider).localeCompare(text(b.provider),"de",{sensitivity:"base"});
      if(mode==="provider_desc")return text(b.provider).localeCompare(text(a.provider),"de",{sensitivity:"base"});
  ...
  function sortCombinedRateRows(){
    const tbody=document.getElementById("rateRows");if(!tbody)return;
    const mode=text(document.getElementById("rateSort")?.value||"newest");
    const trs=[...tbody.querySelectorAll("tr")].filter(tr=>!tr.querySelector(".empty-state"));
    trs.sort((a,b)=>{
      const ap=text(a.dataset.sortProvider),bp=text(b.dataset.sortProvider);
      const ad=new Date(a.dataset.sortDate||0).getTime()||0,bd=new Date(b.dataset.sortDate||0).getTime()||0;
      if(mode==="provider")return ap.localeCompare(bp,"de",{sensitivity:"base"})||(bd-ad);
      if(mode==="provider_desc")return bp.localeCompare(ap,"de",{sensitivity:"base"})||(bd-ad);
      if(mode==="oldest")return ad-bd||ap.localeCompare(bp,"de",{sensitivity:"base"});
      return bd-ad||ap.localeCompare(bp,"de",{sensitivity:"base"});
    });
  ...
  function buildBenchmarkData(rateRows,zoneRows){
    ...
    blocks.sort((a,b)=>a.order-b.order).forEach((block,blockIndex)=>{
      ...
      const emittedZoneRules=new Set();
      block.zones.slice().sort((a,b)=>{
        const ao=Number(a.manualOrder),bo=Number(b.manualOrder);
        if(Number.isFinite(ao)&&Number.isFinite(bo)&&ao!==bo)return ao-bo;
        const ac=text(a.destCountry),bc=text(b.destCountry);if(ac!==bc)return ac.localeCompare(bc);
        return text(a.destPostcodeFrom).localeCompare(text(b.destPostcodeFrom),undefined,{numeric:true});
      }).forEach((z,i)=>{
        const zk=text(z.zone)||text(z.destinationName)||text(z.sourceRef);
        let n=Number(z.manualZoneNumber);
        if(!Number.isInteger(n)||n<1)n=zoneNo.get(zk);
        if(!Number.isInteger(n)||n<1){
          const sampleKey=zoneKeys.find(k=>text(k)===zk);n=sampleKey?zoneNo.get(sampleKey):i+1;
        }
        if(n>MAX_ZONES)return;
        const sig=[text(z.originCountry),text(z.originPostcodeFrom),text(z.originPostcodeTo),text(z.destCountry),text(z.destPostcodeFrom),text(z.destPostcodeTo),n].join("|");
        if(emittedZoneRules.has(sig))return;emittedZoneRules.add(sig);
        zoneOut.push([provider,text(z.originCountry)||"#ALL",text(z.originPostcodeFrom)||0,text(z.originPostcodeTo)||99999,text(z.destCountry)||"#ALL",text(z.destPostcodeFrom)||0,text(z.destPostcodeTo)||99999,n]);
      });
      /* Nur wenn für eine Preiszone tatsächlich keine Zonenregel gespeichert ist,
         bleibt ein expliziter Fallback. So entstehen nicht mehr pauschal 0–99999
         für jede vorhandene Tarifzone. */
      zoneKeys.forEach((zk,i)=>{
        const n=zoneNo.get(zk)||i+1;
        if(block.zones.some(z=>(Number(z.manualZoneNumber)===n)||text(z.zone)===zk))return;
        const sample=zoneSamples.get(zk)||{};
        zoneOut.push([provider,text(sample.originCountry)||"#ALL",0,99999,text(sample.destCountry)||"#ALL",0,99999,n]);
      });
    });
